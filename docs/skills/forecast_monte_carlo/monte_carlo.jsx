...
// ── INJECTED DATA ─────────────────────────────────────────────────────────────
// MCP_RESPONSE is not used directly — both mode objects are in CHART_ATTRS.
// Kept for pattern consistency; str_replace it with {} or the last tool result.
...
const DURATION = CHART_ATTRS.duration || null;
const SCOPE    = CHART_ATTRS.scope    || null;
...
export default function MonteCarloChart() {
  ...
  const active = isDuration ? DURATION : SCOPE;

  if (!active) return <div style={{ color: ALARM, padding: 40 }}>No forecast data available.</div>;

  const ctx = active.context;
  const tc  = trendColor(active.throughput_trend.direction);

  // Percentile chart data
  const percData = PERC_ORDER.map(key => ({
    key,
    label: PERC_SHORT[key],
    value: active.percentiles[key],
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 14 }}>
          ...
          <StatCard label="THROUGHPUT (RECENT)" value={`${ctx.throughput_recent.toFixed(2)}/d`}
            sub={`${active.throughput_trend.direction} · ${active.throughput_trend.percentage_change >= 0 ? "+" : ""}${active.throughput_trend.percentage_change.toFixed(0)}%`}
            color={tc} />
        ...
        {(active.warnings || []).length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
            {active.warnings.map((w, i) => <Badge key={i} text={`⚠ ${w}`} color={CAUTION} />)}
          </div>
        )}
        ...
        {bothModes && (
          <div style={{ display: "flex", marginBottom: 16 }}>
            <div style={{ display: "flex", borderRadius: 8, overflow: "hidden",
              ...
              <button onClick={() => setMode("duration")} style={{
                padding: "8px 16px", fontSize: 11, cursor: "pointer",
                background: isDuration ? `${PRIMARY}33` : "transparent",
                color: isDuration ? TEXT : MUTED, border: "none",
                borderRight: `1px solid ${BORDER}`,
                fontFamily: "'Courier New', monospace",
              }}>Duration — when will {DURATION.composition.total} items finish?</button>
              <button onClick={() => setMode("scope")} style={{
                padding: "8px 16px", fontSize: 11, cursor: "pointer",
                background: !isDuration ? `${PRIMARY}33` : "transparent",
                color: !isDuration ? TEXT : MUTED, border: "none",
                fontFamily: "'Courier New', monospace",
              }}>Scope — how much in {SCOPE.target_days} days?</button>
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
          {isDuration && DURATION && (
            <>
              <StatCard label="TOTAL ITEMS" value={DURATION.composition.total} color={TEXT} />
              <StatCard label="BACKLOG" value={DURATION.composition.existing_backlog}
                sub="unstarted" color={CAUTION} />
              <StatCard label="WIP" value={DURATION.composition.wip}
                sub="in progress" color={PRIMARY} />
              {DURATION.composition.additional_items > 0 && (
                <StatCard label="ADDITIONAL" value={DURATION.composition.additional_items}
          ...
          {!isDuration && SCOPE && (
            <>
              <StatCard label="TARGET WINDOW" value={`${SCOPE.target_days}d`}
                sub={`≈ ${Math.round(SCOPE.target_days / 7)} weeks`} color={SECONDARY} />
              <StatCard label="P85 DELIVERY" value={`${SCOPE.percentiles.likely} items`}
                sub="likely outcome" color={POSITIVE} />
              <StatCard label="P50 DELIVERY" value={`${SCOPE.percentiles.coin_toss} items`}
        ...
        <div style={{ background: PANEL_BG, borderRadius: 12,
          ...
          <div style={{ fontSize: 11, color: MUTED, marginBottom: 8 }}>
            {isDuration
              ? `Days to complete all ${DURATION?.composition?.total || "?"} items · lower = faster`
              : `Items deliverable within ${SCOPE?.target_days || "?"} days · higher = more delivery`}
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={percData} layout="vertical"
              ...
              <Tooltip content={<PercTooltip isDuration={isDuration} labels={active.labels} />}
              ...
              <ReferenceLine x={active.percentiles.likely} stroke={CAUTION} strokeDasharray="4 4"
                label={{ value: `P85: ${active.percentiles.likely}${isDuration ? "d" : ""}`, position: "top",
        ...
        {/* Spread panel */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
          <StatCard label="PREDICTABILITY" value={active.predictability}
            color={predictabilityColor(active.predictability)} />
          <StatCard label="IQR SPREAD" value={active.spread.iqr}
            sub="interquartile range" color={CAUTION} />
          <StatCard label="INNER 80" value={active.spread.inner_80}
            sub="P10–P90 span" color={CAUTION} />
          <StatCard label="FAT-TAIL RATIO" value={`${active.fat_tail_ratio}×`}
            sub="tail / median" color={fatTailColor(active.fat_tail_ratio)} />
        </div>
              ...
              <tbody>
                {strat.map((s, i) => (
                  <tr key={s.type} style={{ background: i % 2 === 0 ? "transparent" : `${PRIMARY}05` }}>
                    <td style={{ padding: "5px 8px", color: s.eligible ? TEXT : MUTED }}>{s.type}</td>
                    <td style={{ padding: "5px 8px", color: s.eligible ? POSITIVE : ALARM }}>
                      {s.eligible ? "Yes" : "No"}</td>
                    <td style={{ padding: "5px 8px" }}>{s.volume}</td>
                    <td style={{ padding: "5px 8px" }}>{s.p85_cycle_time?.toFixed(1) || "—"}d</td>
                    <td style={{ padding: "5px 8px", color: MUTED, fontSize: 10 }}>
        ...
        <div style={{ fontSize: 11, color: MUTED, lineHeight: 1.7,
          ...
          <div style={{ marginBottom: 6 }}>
            ...
            {isDuration && DURATION
              ? `Duration mode forecasts when ${DURATION.composition.total} items will be completed based on historical throughput. Bars show days to completion at each confidence level — shorter is faster. P85 (${DURATION.percentiles.likely}d) is the professional commitment standard.`
              : SCOPE
              ? `Scope mode forecasts how many items will be delivered within ${SCOPE.target_days} days. Bars show item counts — higher means more delivery. P85 (${SCOPE.percentiles.likely} items) is the professional commitment standard. Note the axis is intentionally reversed from duration mode: higher confidence = fewer items guaranteed.`
              : ""}
          </div>
          <div>
