...
const STATUS_ORDER  = CHART_ATTRS.status_order;
    ...
    <div style={{ background: PAGE_BG, minHeight: "100vh", padding: "24px 20px",
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 14 }}>
          ...
          <StatCard label="OLDEST ITEM"      value={`${oldest.wip.toFixed(0)}d`}
            sub={oldest.key}                                                        color={ALARM} />
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 20, alignItems: "center" }}>
          ...
          <Badge text={`Little's Law: ${STABILITY.toFixed(2)} — ${si.label}`} color={si.color} />
          <div style={{ flex: 1 }} />
              ...
              <tbody>
                {outlierRows.map((d, i) => (
                  <tr key={d.key} style={{ background: i % 2 === 0 ? "transparent" : `${PRIMARY}05` }}>
                    <td style={{ padding: "5px 8px", color: outlierColor(d.wip), fontWeight: 700 }}>{d.key}</td>
                    <td style={{ padding: "5px 8px", color: typeColorMap[d.type] || MUTED }}>{d.type}</td>
                    <td style={{ padding: "5px 8px", color: MUTED, fontSize: 10 }}>{d.status}</td>
                    <td style={{ padding: "5px 8px", color: outlierColor(d.wip), fontWeight: 700 }}>{d.wip.toFixed(1)}d</td>
                    <td style={{ padding: "5px 8px", color: SECONDARY }}>{d.statusAge}d</td>
                    <td style={{ padding: "5px 8px", color: ALARM }}>P{d.pct}</td>
                  </tr>
                ))}
              </tbody>
            </table>
