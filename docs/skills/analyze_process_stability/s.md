...
description: >
  Creates a dark-themed React/Recharts Cycle Time Scatterplot for the Process Stability
  analysis (mcs-mcp:analyze_process_stability). Trigger on: "process stability chart",
  ...
  ONLY for analyze_process_stability output. Do NOT use for: throughput stability
...
# Process Stability Chart — Template Skill
...
1. Ensure `mcs-mcp:analyze_process_stability` has been called and its result is present
   in the conversation. `include_raw_series` is optional — the template works correctly
...
|---|---|---|
| `RAW_SCATTERPLOT` | `data.scatterplot[]` | Full array — keep all fields: `date`, `value`, `moving_range`, `key`, `issue_type` |
| `OVERALL_XMR` | `data.stability.xmr` | Object with `average`, `upper_natural_process_limit`, `lower_natural_process_limit`, `signals[]` |
| `OVERALL_SI` | `data.stability.stability_index` | Single number |
| `OVERALL_ELT` | `data.stability.expected_lead_time` | Single number (estimated P50 in days) |
| `STRATIFIED` | `data.stratified` | Object keyed by issue type name; each entry has `.xmr` (with `signals[]`), `.stability_index`, `.expected_lead_time` |
| `BOARD_ID` | `context.board_id` | Integer |
| `PROJECT_KEY` | `context.project_key` | String |
| `BOARD_NAME` | Known from `import_boards` result | String |

**Signal object shape** (used inside `OVERALL_XMR.signals` and `STRATIFIED[type].xmr.signals`):
```js
{ index: number, key: "PROJ-123", type: "outlier" | "shift" }
```

**Note on `include_raw_series`:** This parameter is optional. The template uses only
`data.scatterplot[]` for chart data — it never reads `data.stability.xmr.values[]` or
`moving_ranges[]`. The response structure and all injectable fields are identical
regardless of whether `include_raw_series` is `true` or `false`.
...
const CFG = {
  ...
  siClogged:         1.3,   // SI > this → clogged (red)
  siMarginal:        0.9,   // SI > this → marginal (amber); below → stable (green)
  ...
  // Palette for dynamic issue type color assignment (cycles if more types than entries)
...
// ── DATA (INJECT) ─────────────────────────────────────────────────────────────
// Replace the values below with data from the mcs-mcp:analyze_process_stability
...
const RAW_SCATTERPLOT = [
  // { date: "YYYY-MM-DD", value: 0, moving_range: null, key: "PROJ-1", issue_type: "Story" },
];
...
const OVERALL_XMR = {
  ...
  signals: [
    // { index: 0, key: "PROJ-1", type: "outlier" | "shift" }
  ],
};

const OVERALL_SI  = 0;   // data.stability.stability_index
const OVERALL_ELT = 0;   // data.stability.expected_lead_time

const STRATIFIED = {
  // Keyed by issue type name exactly as it appears in data.stratified.
  // Example entry:
  // Story: {
  //   xmr: {
  //     average: 0,
  //     upper_natural_process_limit: 0,
  //     lower_natural_process_limit: 0,
  //     signals: [ { index: 0, key: "PROJ-1", type: "outlier" } ],
  //   },
  //   stability_index: 0,
