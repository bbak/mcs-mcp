...
export default function BacktestChart() {
  ...
  if (!active) return <div style={{ color: ALARM, padding: 40 }}>No backtest data available.</div>;
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        {bothModes && (
          <div style={{ display: "inline-flex", borderRadius: 6, overflow: "hidden",
            ...
            }}>Duration — days to completion</button>
            <button onClick={() => setMode("scope")} style={{
              padding: "8px 16px", fontSize: 11, cursor: "pointer",
              background: !isDuration ? `${PRIMARY}33` : "transparent",
              color: !isDuration ? TEXT : MUTED, border: "none",
              fontFamily: "'Courier New', monospace",
            }}>Scope — items per window</button>
          </div>
        )}

        {/* Stat cards */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
          <StatCard label="ACCURACY" value={`${Math.round(active.accuracy_score * 100)}%`}
            color={accuracyColor(active.accuracy_score)} />
          <StatCard label="WITHIN CONE" value={`${active.hits} / ${active.total}`}
            sub="checkpoints" color={PRIMARY} />
          <StatCard label="MISSES" value={active.total - active.hits}
            sub="outside cone" color={ALARM} />
          <StatCard label="DRIFT SIGNALS"
            value={active.checkpoints.filter(c => c.drift).length}
            sub="process shifts"
            color={active.checkpoints.filter(c => c.drift).length > 0 ? ALARM : POSITIVE} />
          <StatCard label="MODE" value={isDuration ? "Duration" : "Scope"}
            sub={isDuration ? "days to completion" : "items per window"} color={SECONDARY} />
        </div>

        <BacktestPanel data={active} isDuration={isDuration} />
        ...
        <div style={{ fontSize: 11, color: MUTED, lineHeight: 1.7,
          ...
          <div style={{ marginBottom: 6 }}>
            ...
            confidence.
            {bothModes && ` A low duration score (${Math.round(DURATION.accuracy_score * 100)}%) combined with a higher scope score (${Math.round(SCOPE.accuracy_score * 100)}%) typically indicates that throughput volume is more predictable than individual item cycle times.`}
          </div>
          <div>
